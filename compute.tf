...
resource "null_resource" "ansible" {
  ...
  provisioner "local-exec"{
    command = "sudo ansible-playbook -i ${oci_core_instance.TestInstance[count.index].public_ip}, ansible/redis-playbook.yml --extra-vars variable_host=${oci_core_instance.TestInstance[count.index].public_ip}"
  }
}
